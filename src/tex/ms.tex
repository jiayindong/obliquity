...
Furthermore, the stellar obliquity distribution of misaligned Hot Jupiters presents a broad distribution from $0\degr$ to $180\degr$, preferring candidate Hot Jupiter formation mechanisms, such as planet-planet scattering after a convergent disk migration or planetary Kozai, that could produce a broad distribution.}
...
In this section, we find the expression of $\lambda$ or $i_\star$ in terms of stellar obliquity $\psi$, the azimuthal angle of the stellar spin axis relative to the orbital axis $\theta$, and the orbital inclination $i_{\rm orb}$. In Figure~\ref{fig:coord}, we introduce two coordinate systems that describe the stellar spin axis $\vb{n_\star}$ and its planet's orbital axis $\vb{n_{\rm orb}}$. The setup is similar to the coordinate systems used in \cite{Fabrycky09} but has different coordinate orientations and definitions of the azimuthal angle of the stellar spin.

The angle of primary interest is the angle between the planet's orbital axis and its host star's spin axis $\psi$. To describe the angle in 3$D$, we introduce another angle, the azimuthal angle of the stellar spin around the orbital axis $\theta$. As shown in the left panel in Figure~\ref{fig:coord}, in the $\{\psi, \theta\}$ coordinate system, we put the planetary orbital axis on the $\vb{z}$-axis. The stellar spin axis, described by $\psi$ and $\theta$, can be written as 
\begin{equation}
    \vb{\hat{n}_\star} = \sin{\psi}\cos{\theta}\vu*{x} + \sin{\psi}\sin{\theta}\vu*{y} + \cos{\psi}\vu*{z}
\end{equation}

Next, we introduce an observer-oriented coordinate system. Since we neither know the projected orbital axis in the sky plane nor the projected stellar spin axis, we conveniently set the projected orbital axis to the $\vb*{z}$-axis. In the $\{\lambda, i_\star\}$ coordinate system, shown in the right panel of Figure~\ref{fig:coord}, we point the $\vb{x'}$-axis to the observer. The stellar spin axis could be written as 
\begin{equation}\label{eqn:nstar1}
    \vb{\hat{n}_\star}' = \cos{i_\star}\vu*{x}' + \sin{i_\star}\sin{\lambda}\vu*{y}' + \sin{i_\star}\cos{\lambda}\vu*{z}'
\end{equation}

To transform the stellar spin vector $\vb{\hat{n}_\star}$ from the $\vb*{xyz}$ coordinate to the $\vb*{x'y'z'}$ coordinate, we rotate the $\vb*{xyz}$ coordinate by an angle $\pi/2 - i_{\rm orb}$ about the $\vb{y'}$-axis (the same as $\vb{y}$-axis). 
...
\end{equation}
we can now find the expression of $\vb{\hat{n}_\star}'$ in the $\vb*{x'y'z'}$ coordinate in terms of $\psi$, $\theta$, and $i_{\rm orb}$:
...
Comparing Equation~(\ref{eqn:nstar2}) with Equation~(\ref{eqn:nstar1}), we find the expression of $\lambda$ or $i_\star$ in terms of $\psi$, $\theta$, and $i_{\rm orb}$.
First, from the $\vb{\hat{x}'}$ terms, we derive
\begin{equation}
    i_\star = \cos[-1](\sin{\psi}\cos{\theta}\sin{i_{\rm orb}}+\cos{\psi}\cos{i_{\rm orb}}).
\end{equation}
Next, dividing the $\vb{\hat{y}'}$ terms by the $\vb{\hat{z}'}$ terms, we find
\begin{equation}
...
\edits{In this framework, we limit $i_{\rm orb}$ from $0\degr$ to $90\degr$. To preserve the symmetry, we allow $i_{\rm orb}$ to vary from $0\degr$ to $180\degr$.}
With the design, the hyperparameter and parameter priors are the following:
\begin{align}
    w_{0,1} &\sim {\rm Dirichlet}(1, 1) \nonumber\\
    p_{0,1} &\sim \mathcal{U}(0, 1) \nonumber\\
    q_{0,1} &\sim \mathcal{U}(0, 1) \nonumber\\
