...
Applying the framework to all exoplanetary systems with sky-projected stellar obliquity measurements, we find that the stellar obliquity distribution is unimodal and peaked at zero degrees. Misaligned systems have nearly isotropic stellar obliquities, which may have important implications for close-in planets' formation and evolution.
...
Stellar inclinations can often be constrained via, e.g., photometric and spectroscopic rotational modulation introduced by starspots \citep[e.g.,][]{Masuda20, Albrecht21}, gravity darkening \citep[e.g.,][]{Barnes09, Barnes11}, and asteroseismology \citep[e.g.][]{Chaplin13}.
...
We do not yet understand the role of stellar inclination in population stellar obliquity inference.
In this work, we study the relationship between the distributions of stellar obliquity, sky-projected stellar obliquity, and stellar inclination, and introduce a statistical approach to infer the stellar obliquity distribution.
In Section~\ref{sec:hbm}, we introduce a flexible, hierarchical Bayesian framework that infers the stellar obliquity distribution of a sample.
In Section~\ref{sec:applications}, we apply the framework to simulated data and real observations and show that the stellar obliquity distribution inferred purely from sky-projected stellar obliquities will not compromise our interpretation.
In Section~\ref{sec:jacobian}, we find the mathematical relation between the $\psi$, $\lambda$, and $i_\star$ distributions and explain why the stellar obliquity distribution inference is predominantly determined by sky-projected stellar obliquities.
...
This study was carried out with the reproducible workflow \href{https://github.com/showyourwork/showyourwork}{\showyourwork} \citep{Luger2021}, which leverages continuous integration to programmatically download the data from \href{https://zenodo.org/}{zenodo.org}, create the figures, and compile the manuscript. 
...
A Jupyter notebook demonstration including case studies of the stellar obliquity distribution inference can be found in the same repository.
